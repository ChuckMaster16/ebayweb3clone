  ...
  return (
    <div>
        ...
        <main className='max-w-6xl mx-auto pt-2'>
            ...
            <hr className='mb-5'/>
            {address ? (<p>Below you will find the NFTs in which you own in your wallet</p>):(
                <div className='text-center py-5'>
                    <p>You are not Loged in please connect your Wallet</p>
                    <p>in order to list Items and mint your NFTs</p>
                    <button 
                    onClick={connectWithMetamask}
                    className='bg-blue-500 p-2 rounded-lg mt-6 text-white font-semibold shadow-lg active:bg-white active:text-blue-500'> Connect your wallet</button>
                </div>
            ...
            <div className='flex overflow-x-scroll space-x-2  p-4'>
                {ownedNfts?.data?.map((nft) => (
                    ...
                    className={`flex flex-col space-y-2 card min-w-fit border-2 bg-gray-100 ${nft.metadata.id === selectNft?.metadata.id ? 'border-black scale-105 shadow-md' : 'border-transparent'}`}>
                        <MediaRenderer
                ...
                <form action="">
                    <div className='flex justify-center flex-col p-10'>
                        <div className=" grid grid-cols-2 p-10 gap-5">
                            ...
                            <input className='bg-gray-100 p-5 rounded-lg' name='price' type="text" placeholder='0.05' />
                        </div>
                        <button 
                        type='submit'
                        onSubmit={handleCreateListing}
                        className='bg-blue-500 p-3 text-white font-semibold rounded-2xl mt-8 shadow-lg boder hover:bg-white hover:text-blue-500 active:text-white active:bg-blue-400'>List Items</button>
                    </div>
                </form>
            )}
        </main>
